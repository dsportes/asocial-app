      ...
      <div v-for="nom in note.lstNoms" :key="nom" class="full-width q-mb-sm">
        ...
        <div v-for="e in mpn.get(nom)" :key="e.f.idf" class="q-ml-lg q-my-xs">
          ...
          <div v-if="e.f.thn" class="row justify-center q-mt-xs q-mb-md">
            <img class="q-my-sm bordimg cursor-pointer"
              :src="e.f.thn" @click="affImg(e.f)"/>
          </div>
        </div>
...
const vgr = computed(() => !groupe.value ? 0 : groupe.value.alVolGr(0))

// volume fichier du compte (si hébergeur pour un groupe)
const vcpt = computed(() => !groupe.value || (groupe.value && !groupe.value.cptEstHeb) ? 0 : session.alVolCpt(0))
