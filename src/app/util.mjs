...
/*
Obtient le statut de blocage depuis tribu / compta
- l'enregistre dans session.blocage
- si c'est une connexion ou une création de compte (donc pas une synchro)
  ET que on constate un DEBLOCAGE, la session va être reconnectée
  ce qui provoque une alerte à confirmer
*/
export async function getBlocage (tribu, compta, depuisSynchro) {
  const $q = stores.config.$q
  return new Promise((resolve) => {
    ...
    const av = session.blocage || 0 // celui courant (voire mis en debug)
    let ap = 0
    if (tribu && (tribu.stn > ap)) ap = tribu.stn
    if (compta && (compta.stn > ap)) ap = compta.stn
    if (av !== ap) session.blocage = ap
    if (!depuisSynchro) {
      resolve()
      return
    }
