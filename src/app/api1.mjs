...
export class Compteurs {
  ...
  aboma : somme des coûts d'abonnement des mois antérieurs au mois courant depuis le début de la période de référence
  consoma : somme des coûts consommation des mois antérieurs au mois courant depuis le début de la période de référence
  ...
  */
  ...
  get njec () {
    ...
    return sc <= 0 ? 0 : Math.floor(sc / (this.cjAbo + this.qv.cjm))
  }
  ...
  get soldeCourant () { 
    return this.soldeP + this.creditsP - this.donsP - (this.estA ? (this.aboma + this.consoma) : 0) 
  }
  ...
  setA (estA) { 
    ...
    this.soldeP = this.soldeP + this.creditsP - this.donsP - (this.estA ? this.cumulCouts : 0) 
    this.estA = estA
    this.dhP = this.dh
    this.donsP = 0
    this.creditsP = 0
  ...
  majDhsn () {
    if (!this.estA) return // compte O : dhsn inchangée,  (gratuit)
