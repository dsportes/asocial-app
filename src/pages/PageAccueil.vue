<template>
  <q-page>
    <div class="q-pa-sm">
      <div class="q-my-xs titre-md text-center text-bold text-italic">{{$t('ACav', [session.avC.na.nomc])}}</div>
    ...
    <div :class="'q-pa-sm' + (!session.groupeId ? ' disabled' : '')">
      <div class="q-my-xs titre-md text-center text-bold text-italic">
    ...
    <q-separator class="q-my-sm"/>
      <div class="row items-center justify-center q-gutter-sm">
        ...
        <q-btn class="btn1" dense no-caps color="primary" :label="$t('ACmesspons')">
          <q-badge color="teal-10" rounded floating>{{nbtsp}}</q-badge>
        </q-btn>
        <q-btn class="btn1" dense no-caps color="primary" :label="$t('ACmesinvits')">
          <q-badge color="teal-10" rounded floating>{{nbtiv}}</q-badge>
        </q-btn>
      </div>
      <div class="full-width q-mt-sm column items-center q-gutter-sm">
        <q-btn class="btn3" dense no-caps color="primary" :label="$t('ACmessecrets')" />
      </div>
    <q-separator class="q-my-sm"/>
      <div class="q-my-xs titre-md text-center text-bold text-italic">{{$t('ACinfs', [session.tribu.na.nom])}}</div>
      <div v-if="session.compte && session.compte.estParrain" 
        class="q-my-xs titre-md text-center text-bold text-italic text-warning">{{$t('ACcptspons')}}</div>
      <div class="q-mt-sm row items-center justify-center q-gutter-sm">
      ...
      <div :class="'q-mt-sm ' + (!session.accesIdb ? ' disabled' : '')" class="column items-center q-gutter-sm">
